...
func handleConnection(conn net.Conn, sendPort string) {
	defer conn.Close()
	...
	for {
		...
		if out, error := httputil.DumpRequest(req, true); error == nil {
			log.Println("Request\n", string(out[:]))
		}

		be, err := getBackend(sendPort)
		if err != nil {
			return
		}
		...
		if err := req.Write(be.Writer); err == nil {
			...
			if resp, err := http.ReadResponse(be.Reader, req); err == nil {
				...
				if response, error := httputil.DumpResponse(resp, true); error == nil {
...
func getBackend(toPort string) (*Backend, error) {
	tosend := fmt.Sprintf("127.0.0.1:%s", toPort)
	select {
	case be := <-backendQueue:
		return be, nil
	case <-time.After(100 * time.Millisecond):
		be, err := net.Dial("tcp", tosend)
		if err != nil {
			return nil, err
		}
